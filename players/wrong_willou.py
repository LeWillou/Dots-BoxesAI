...
class model:
	...
	def play(self, board, available_cells, is_first_player):
		...
		if len(self.q_table) == 0:
			...
			cell_to_choose = choice(available_cells[1:6])
			...
			for i in range(len(self.q_table)):
				if state in self.q_table:
					index_to_look_at = self.q_table.index(state)
					choice_list.append(self.q_table[index_to_look_at])
			...
			if is_found == 1:
				self.current_choice = choice_list[len(choice_list)]
